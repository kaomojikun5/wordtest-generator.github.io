...
const TARGET1900_JSON = __dirname + "/target1900.json";
...
(async (
  ...
  const target1900 = await (async (cache) => {
    try {
      return JSON.parse(await cache);
    } catch (e) {
      const URL = "https://ukaru-eigo.com/target-1900-word-list/";
      console.log("ターゲット１９００データを ${URL} からダウンロード中...");
      const target1900 = (await tabletojson.convertUrl(URL))[0].map((word) => ({
        id: parseInt(word["番号"]),
        en: word["単語"],
        jp: word["意味"],
      }));
      await fs.promises.writeFile(
        TARGET1900_JSON,
        JSON.stringify(target1900, null, 2) + "\n",
        "UTF8"
      );
      return target1900;
    }
  })(fs.promises.readFile(TARGET1900_JSON, "UTF8"));

  const [l, r] = minmax(
    clamp(new Number(left), 1, target1900.length),
    clamp(new Number(right), 1, target1900.length)
  );
  const n = clamp(new Number(num), 1, r - l + 1);
  const s = parseInt(seed, 16) || Math.floor(Math.random() * 2 ** 16);
  const title = `ターゲット１９００テスト ${l}~${r} (SEED:${s
    .toString(16)
    .toUpperCase()
    .padStart(4, "0")})`;
